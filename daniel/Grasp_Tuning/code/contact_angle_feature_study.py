...
from helpers import v3_contact_region, cv_plot_contact_region, extract_contact_points, contact_angles
...
def draw_rectangle(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        ...
        if position_change or angle_change:
            img = img2.copy()
            contact_r = v3_contact_region(y1, x1, angle, 19, 130, Im[:, :, 0])
            # contact_ps, n1, n2 = extract_contact_points(contact_r, 8)
            # ang_l, ang_r, ang_v, n_sum = contact_angles(contact_r, angle)
            cv_plot_contact_region(contact_r, img, False)
            plot_grasp_path([y1, x1], angle, 19, 130, img)
            # font = cv2.FONT_HERSHEY_SIMPLEX
            # text = f'contact angles [left, right]: [{ang_l:.3f}, {ang_r:.3f}],{ang_l+ang_r:.3f},{abs(ang_l-ang_r):.3f}, {ang_v:.3f} {n_sum:.3f}'
