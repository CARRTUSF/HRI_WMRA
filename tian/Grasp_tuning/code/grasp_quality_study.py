...
def draw_rectangle(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        ...
        if position_change or angle_change:
            ...
            contact_rl, contact_rr = extract_contact_region(y1, x1, angle, 19, 80, Im[:, :, 0])
            ...
            score2 = grasp_quality_score_v2(cli, trans, rotation, slippage, offs, 19, 80)
            score3 = grasp_quality_score_v2(cli, trans, rotation, slippage, offs, 19, 80, new=True)
            score4 = evaluate_grasp([y1, x1, angle, 19, 80], Im[:, :, 0])
            ...
            plot_grasp_path([y1, x1], angle, 19, 80, img)
            plot_contact_profile(contact_rl[:, 3], img, 'left', x1, y1, angle, 38, 160, lcids)
            plot_contact_profile(contact_rr[:, 3], img, 'right', x1, y1, angle, 38, 160, rcids)
            ...
            # score_text = f'overall scores v1:{scores[0][0] * 100:.2f}+-{scores[0][1] * 100:.3f} %'
            score2_text = f'overall score: {score2:.2f} '    # v3: {score3 * 100:.2f} % , {score4 * 100:.2f} %
            ...
            # cv2.putText(img, score2_text, (150, 220), font, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.putText(img, prediction_text1, (300, 500), font, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.putText(img, prediction_text2, (300, 520), font, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.putText(img, prediction_text3, (300, 540), font, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.putText(img, prediction_text4, (300, 560), font, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.putText(img, prediction_text5, (300, 580), font, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
